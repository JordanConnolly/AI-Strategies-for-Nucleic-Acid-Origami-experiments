...
for j in range(reps):
    ...
    path = "E:/PhD Files/RQ3/final_machine_learning_experiments/Y-Permutation_Validation_Experiments/" \
           "Experiment_Set_2_Magnesium_Permutations_Revnano/" \
           "Experiment_Set_2_Magnesium_Y_Permutation_RevNano_" + experiment_number + "/"
    ...
    all_final_scores_df = pd.DataFrame(columns=["r2", "MAE", "RMSE", "MSE", "MedianAE"])
    ...
    for repetition in range(1, 31):
        ...
        count = 0
        all_fold_scores_df = pd.DataFrame(columns=["r2", "MAE", "RMSE", "MSE", "MedianAE"])
        for file in all_files:
            rep_final_score_fold_df = pd.read_csv(file, names=["r2", "MAE", "RMSE", "MSE", "MedianAE"])
