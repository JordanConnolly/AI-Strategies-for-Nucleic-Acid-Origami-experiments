...
# Overall folder
name_of_root_folder = "./Experiment_Set_2_thermal_Permutations_Revnano" + "/"
try:
...
for i in range(1, 31):
    ...
    name_of_folder = name_of_root_folder + "Experiment_Set_2_thermal_Y_Permutation_RevNano_" + str(i) + "/"
    try:
    ...
    x = final_data_set.drop(columns=[predictor, 'y', 'Thermal Profile', 'Peak Temperature (oC)',
    ...
    permuted_final_dataset.to_csv(name_of_folder + "y_permutation_thermal_experiment_revnano_" + str(i) + "_dataset.csv")
    print("Stored y-permutation dataset:" + str(i))

    # Store the config for dataset
    copy_of_dict = hyper_parameters_dict
    copy_of_dict.update(dataset_filename="/y_permutation_thermal_experiment_revnano_" + str(i) + "_dataset.csv")
    ...
    shutil.copy("extra_trees_RFE_y_permuted_thermal_revnano.py", name_of_folder)

    # create 30 repetition files of the original python file
    for j in range(1, 31):
        shutil.copy("extra_trees_RFE_y_permuted_thermal_revnano.py",
                    name_of_folder + "extra_trees_RFE_y_permuted_thermal_revnano_" + str(j) + ".py")
