...
remove_set_file_path = cwd + '/dot_file_data_set.csv'
remove_set = pd.read_csv(remove_set_file_path)
experiment_to_remove = remove_set['Experiment Number']
experiments_to_remove = experiment_to_remove.to_list()
data_set = data_set[~data_set['Experiment Number'].isin(experiment_to_remove)]

reps = 30
for i in range(reps):
    seed_number = seed_numbers[i]
    data_set = data_set.sample(n=80, replace=True, random_state=seed_number)
