...
sample_weight_df = pd.read_csv("label_data_set_doubled.csv")
# print(sample_weight_df.shape)
sample_weight = sample_weight_df['weights']
...
for n_inner in range(0, len(cols) - features_removed):
    ...
    for i in range(reps):
        ...
        for tr_idx, val_idx in data_splits:
            ...
            weights_train = sample_weight[tr_idx]
            ...
            for n in range(0, 1):
                ...
                ETR.fit(X_train_prep_outer, y_train, sample_weight=weights_train)
                # predict with model
...
experiment_name = "Extra Trees Stratified RevNano"
cross_validation = "3CV Sample Weights"

# Plot labels and Title
fig, ax = plt.subplots()
plt.title(cross_validation + " RRFE " + experiment_name)
plt.xlabel("Number of Most Informative Features Removed")
plt.ylabel("Scoring Metric (r2 value)")

# Make the plot
ax.plot(final_data_frame['number_features_removed'], final_data_frame['train_accuracy'], label="train r2 score",
        alpha=0.5)
ax.plot(final_data_frame['number_features_removed'], final_data_frame['test_accuracy'], label="test r2 score",
        alpha=0.5)
