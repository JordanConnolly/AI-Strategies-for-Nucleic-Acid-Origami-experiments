...
# import the data set
data_set_file_path = cwd + '/correct_imputation_afm_yield_ml_data_set_no_0_25.csv'
...
x = final_data_set.drop(columns=['Yield (%)', 'Yield Range (%)', 'index', 'Unnamed: 0',
...
time_taken_str = "time taken for rep:" + str(rep) + " time:" + str(time_taken)

# Store time taken
with open(important + experiment_name +
          "_time_taken" + ".txt", "a", newline='') as file:
    file.write(f'{time_taken_str}')
    file.close()  # Closes text file
