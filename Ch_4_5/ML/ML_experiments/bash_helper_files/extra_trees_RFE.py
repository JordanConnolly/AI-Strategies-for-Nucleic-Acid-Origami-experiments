...
for tr_idx, val_idx in data_splits:
    ...
    transformed_data = best.named_steps['preprocessor'].transform(x)
    # Gather column names
    x_transformed_columns_outer = get_transformer_feature_names(best['preprocessor'])
    ...
    # IF FEATURE SELECTION -> FE COLUMNS
    ...
    shap_df = pd.DataFrame(shap_values, columns=x_column_numpy)
    shap_df.to_csv(diagnostic + experiment_name +
                   "_SHAP_values_for_plot_" + str(rep) + "_fold_" + str(inner_loop_rep) + '.csv')
    ...
    importance_df.to_csv(important + experiment_name +
                         "_feature_importance_rep_" + str(rep) + "_fold_" +
                         str(inner_loop_rep) + "_best_whole_dataset.csv")
...
clf_all.fit(X=x, y=y)
clf_best = clf_all.best_estimator_

# Get the features from the pipeline
x_transformed_columns_outer = get_transformer_feature_names(clf_best['preprocessor'])
transformed_data = clf_best.named_steps['preprocessor'].transform(x)
df_test = pd.DataFrame(transformed_data, columns=x_transformed_columns_outer)
# IF FEATURE SELECTION -> Feature Elimination COLUMNS
rfe_named = clf_best.named_steps['rfe']
mask = rfe_named.support_
x_column_numpy = np.array(x_transformed_columns_outer)[mask]
...
with open(important + experiment_name +
    ...
    file.write(f'{clf_best}')
    file.close()  # Closes text file

# Feature Importance for whole data fitted set best model -- IMPORTANT --
feature_importance = clf_best.named_steps['estimator'].feature_importances_
feature_importance_list = list(zip(columns_used_names, feature_importance))
importance_df = pd.DataFrame(feature_importance_list, columns=['feature_name', 'importance_value'])
importance_df.sort_values(by=['importance_value'], inplace=True, ascending=False)
importance_df.to_csv(important + experiment_name +
                     "_feature_importance_rep_" + str(rep) + "_best_whole_dataset.csv")

# Explain Shap Importance -- IMPORTANT --
explain = shap.TreeExplainer(clf_best.named_steps['estimator'])
shap_values = explain.shap_values(df_test_fe)
shap_df = pd.DataFrame(shap_values, columns=columns_used_names)
shap_df.to_csv(diagnostic + experiment_name +
               "_SHAP_values_for_plot_" + str(rep) + "_best_whole_dataset.csv")
