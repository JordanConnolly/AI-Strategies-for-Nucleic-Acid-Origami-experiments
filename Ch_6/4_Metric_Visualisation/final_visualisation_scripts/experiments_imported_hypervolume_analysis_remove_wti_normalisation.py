...
for experiment_name in experiment_list:
    ...
    for i in range(1, 11, 1):
        ...
        if rep != 35:
            ...
            #                                str(rep) + "_fits_final_gen.csv")  # load all of the generation score
            ...
            # all_gen_scaffold_df = pd.read_csv(metadata_path + origami_name + "_GA-experiment_rep_" +
            #                                   str(rep) + "_scaffolds_gen_60.csv")
            ...
            # gen_scores = pd.read_csv(results_path + origami_name + "_GA-experiment_rep_" +
            #                          str(rep) + "_fits_gen_60.csv").drop(columns=["Unnamed: 0"])
            ...
            hv_calculated = hv.calc(pareto_front)
    ...
    best_rep_gen_score_df = pd.read_csv(results_path + origami_name + "_GA-experiment_rep_" +
                                        str(best_rep_df["rep"]) + "_fits_gen_250.csv")
    med_rep_gen_score_df = pd.read_csv(results_path + origami_name + "_GA-experiment_rep_" +
                                       str(med_rep_df["rep"]) + "_fits_gen_250.csv")
    # best_rep_gen_score_df = pd.read_csv(results_path + origami_name + "_GA-experiment_rep_" +
    #                                     str(best_rep_df["rep"]) + "_fits_gen_60.csv")
    # med_rep_gen_score_df = pd.read_csv(results_path + origami_name + "_GA-experiment_rep_" +
    #                                    str(med_rep_df["rep"]) + "_fits_gen_60.csv")
    ...
    print("gra norm", best_rep_best_gra_ind)
    ...
    print("gra norm", med_rep_best_gra_ind[0].round(3), med_rep_best_gra_ind[1].round(3), med_rep_best_gra_ind[2].round(3))
