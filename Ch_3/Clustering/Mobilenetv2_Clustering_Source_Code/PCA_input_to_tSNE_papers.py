...
def plot_tsne_results(merged_df, feature_of_interest, init_method, perplexity, n_iter, palette):
    plt.figure(figsize=(10, 8))
    sns.scatterplot(data=merged_df, x="cluster1", y="cluster2", hue=feature_of_interest, legend='full', palette=palette)
    plt.legend(title=feature_of_interest, bbox_to_anchor=(1.20, 1), loc=1)
    # plt.legend(title=feature_of_interest, bbox_to_anchor=(0, 1), loc=3)
    ...
    plt.savefig(os.path.join(output_dir,
                             f"{init_method}_perp_{perplexity}_iter_{n_iter}_"
...
def main():
    ...
    for init_method in init_method_list:
        print(f"Perplexity: {perplexity}, n_iter: {n_iter}, feature of interest: Paper Number")
        ...
        merged_df.sort_values("Paper Number", inplace=True)

        paper_counts = merged_df["Paper Number"].value_counts()
        merged_df["Paper Number Group"] = merged_df["Paper Number"].apply(lambda x: x if paper_counts[x] >= point_threshold else "Others")

        palettes = ["pastel", "coolwarm", "husl", "Set1", "viridis"]
        for palette in palettes:
            plot_tsne_results(merged_df, "Paper Number Group", init_method, perplexity, n_iter, palette)
